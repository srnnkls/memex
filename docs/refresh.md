...
- Search-triggered refreshes on macOS replay the volume's persistent file-system event journal instead of walking. Every committed refresh stores the journal position it captured before reading anything, keyed by the discovery fingerprint plus the roots that existed. The next refresh replays events under the source roots from that position, unions them with files the checkpoint saw modified in the last ten minutes (the journal trails the kernel by a few tens of milliseconds), and resolves the result exactly as the watch daemon resolves its dirty set: only those paths are stat-checked and parsed. A directory rename or removal, a dropped or wrapped event range, a purged journal, a different volume, a root that started existing, or a missing cursor falls back to the stamped walk; explicit `memex index` runs always walk.
...
- Automatic search refresh never merges in the foreground. It appends with `NoMergePolicy`, and when a refresh that added records leaves more than 8 small segments (under 5 % of the corpus, outside the three largest) it spawns one detached `memex index compact` process, which merges those small segments into one and exits. Segments at or above the 5 % share are left alone even when they fall outside the three largest, so a compaction costs a bounded slice of the corpus rather than a rewrite of it. The spawn is skipped while any ingest holds the lease, so at most one compaction runs at a time; no resident process is involved.
