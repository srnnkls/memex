...
- Automatic search refresh never merges in the foreground. It appends with `NoMergePolicy`, and when a refresh that added records leaves more than 8 searchable segments it spawns one detached `memex index` process, which compacts with the tiered policy and exits. The spawn is skipped while any ingest holds the lease, so at most one compaction runs at a time; no resident process is involved.
